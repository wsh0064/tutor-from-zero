# Subject Strategies

## Mathematics And Statistics

- Build prerequisite chains explicitly.
- Explain definitions geometrically or numerically before symbolism.
- State theorem and formula conditions every time they are applied.
- Show derivation decisions, not just algebra.
- Use estimation, units, substitution, or limiting cases to check answers.

## Physics And Engineering

- Begin with the real system, assumptions, variables, and sign convention.
- Separate physical law, model simplification, and calculation.
- Track units throughout.
- Highlight standards or procedural steps whose violation loses substantial marks.
- Use diagrams when spatial relationships matter.

## Computer Science

- Trace small inputs by hand before generalizing.
- Separate interface, invariant, algorithm, and complexity.
- Explain why an implementation is correct and under which assumptions.
- Use code only after the concept is visible; include edge cases and complexity.

## Humanities

- Replace formula derivation with claim, evidence, context, counterargument, and
  qualification.
- Build timelines, causal chains, and concept comparisons.
- Teach answer structure explicitly: thesis, organized evidence, analysis, conclusion.
- Distinguish primary evidence, scholarly interpretation, and the student's synthesis.

## Social Sciences

- Separate description, mechanism, evidence, and normative judgment.
- Compare theories across assumptions, unit of analysis, predicted outcomes, and limits.
- Use cases as tests of theory, not decorative anecdotes.
- Mark empirical uncertainty and avoid presenting correlation as causation.

## Economics, Management, And Accounting

- Pair each formula with its business meaning and decision use.
- Emphasize similar concepts, classification rules, and condition changes.
- Use short cases and transaction flows.
- Show calculations in a mark-friendly sequence and interpret the final value.

## Languages And Literature

- Combine retrieval, production, and correction rather than passive rereading.
- For language learning, balance vocabulary, grammar, listening or reading, and output.
- For literature, connect close textual evidence to an arguable interpretation.
- Avoid reproducing long copyrighted passages; use short excerpts and paraphrase.

## Practical And Laboratory Courses

- Teach purpose, equipment or inputs, ordered procedure, expected observation, error
  sources, and safety.
- Distinguish conceptual misunderstanding from procedural noncompliance.
- Use checklists for high-consequence steps, but still ask why each step exists.
